      ...
      <div className="flex-1 flex flex-col items-start justify-start gap-4">
        ...
        <div className="flex flex-row items-center justify-start gap-5 text-sm text-[#9CA3AF]">
          <button
            ...
            className={`flex flex-row items-center justify-start gap-1.5 transition-colors ${post.isLiked ? "text-red-500" : "text-[#9CA3AF] hover:text-red-500"}`}
          >
            <Heart className={`w-5 h-5 ${post.isLiked ? "fill-red-500 text-red-500" : ""}`} />
            <span className={`relative tracking-[-0.03em] leading-7 text-sm ${post.isLiked ? "text-red-500" : "text-[#9CA3AF]"}`}>{post.likes}</span>
          </button>
          <button
            onClick={() => window.location.href = `/post/${post.id}#comments`}
            className="flex flex-row items-center justify-start gap-1.5 hover:text-blue-500 transition-colors text-[#9CA3AF]"
          >
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm p-4 flex flex-col gap-3 w-full">
        ...
        <div className="flex flex-row items-center gap-6 pt-1">
          <button
            onClick={() => onLike(post.id)}
            className={`flex items-center gap-1.5 transition-colors ${post.isLiked ? "text-red-500" : "text-[#9CA3AF] hover:text-red-500"}`}
          >
            <Heart className={`w-5 h-5 ${post.isLiked ? "fill-red-500 text-red-500" : ""}`} />
            <span className={`text-sm ${post.isLiked ? "text-red-500" : "text-[#9CA3AF]"}`}>{post.likes}</span>
          </button>
          <button
            onClick={() => window.location.href = `/post/${post.id}#comments`}
            className="flex items-center gap-1.5 hover:text-blue-500 transition-colors text-[#9CA3AF]"
          >
            <MessageCircle className="w-5 h-5" />
